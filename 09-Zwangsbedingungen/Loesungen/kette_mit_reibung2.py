...
Die Kette besteht aus 23 Punktmassen und es wird eine
...
def kettenlinie(laenge, breite, n=500):
    """Berechne eine ideale Kettenlinie.

    Eine Kette der gegebenen Länge wird zwischen den Punkten
    (x=0, y=0) und (x=breite, y=0) aufgehängt.
    ...
    Args:
        ...
        n (int):
            Anzahl der zu berechnenden Stützstellen.

    Returns:
        tuple[np.ndarray, np.ndarray]:
            - x-Werte der Kettenlinie.
            - y-Werte der Kettenlinie.
    """
    # Wir beschreiben die Kettenlinie durch die folgende Funktion:
    #
    #     y(x) = a * cosh((x - breite/2) / a) + h
    #
    # Die Berechnung der Bogenlänge der Funktion y(x) liefert:
    #     laenge = breite * sinh(u) / u
    # mit
    #     u = breite / (2 * a) .

    # Bestimme u über die numerische Lösung der Gleichung
    #      sinh(u) / u - laenge / breite = 0.
    def func(u):
        return np.sinh(u) / u - laenge / breite
    res = scipy.optimize.root(func, 1.0)
    u = res.x[0]

    # Bestimme aus dem Wert von u den Parameter a.
    a = breite / (2 * u)

    # Berechne die Verschiebung h der Kurve aus der Bedingung
    # y(0) = 0.
    h = -a * np.cosh(-breite / 2 / a)

    # Lege ein Array von x-Werte an, werte die Kettenlinie aus und
    # gib das Ergebnis zurück.
    x = np.linspace(0, breite, n)
    y = a * np.cosh((x - breite/2) / a) + h
    return x, y


# Plotte die ideale Kettenlinie.
kettenlaenge = np.sum(laengen)
spannbreite = punkte[-1, 0] - punkte[0, 0]
x, y = kettenlinie(kettenlaenge, spannbreite)
ax.plot(x, y, '--r')

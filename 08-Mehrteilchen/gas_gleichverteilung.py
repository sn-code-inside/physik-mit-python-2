...
radien = 0.01 + 0.04 * np.random.rand(n_teilchen)

# Wähle zufällige Massen zwischen 0,01 kg un 1,00 kg.
m = 0.01 + 0.99 * np.random.rand(n_teilchen)
...
ax_teilchen = fig.add_subplot(1, 2, 1)
ax_teilchen.set_title('Teilchenbewegung')
ax_teilchen.set_xlabel('$x$ [m]')
ax_teilchen.set_ylabel('$y$ [m]')
ax_teilchen.set_xlim(-2.1, 2.1)
ax_teilchen.set_ylim(-2.1, 2.1)
ax_teilchen.grid()
...
for radius, masse in zip(radien, m):
    ...
    ax_teilchen.add_patch(kreis)
    kreise.append(kreis)

# Erzeuge eine zweite Axes für die kinetische Energie.
ax_energie = fig.add_subplot(1, 2, 2)
ax_energie.set_title('Energieverteilung')
ax_energie.set_xlabel('$m$ [kg]')
ax_energie.set_ylabel('$E$ [J]')
ax_energie.set_ylim(0, E_max)
ax_energie.set_xlim(0, np.max(m))
ax_energie.grid()

# Erzeuge einen Plot für die kinetische Energie
# als Funktion der Teilchenmasse.
plot_energie, = ax_energie.plot([], [], 'o')

# Erzeuge einen Plot für die Ausgleichsgerade.
plot_gerade, = ax_energie.plot([], [], '-', linewidth=3)
...
def update(n):
    ...
    plot_energie.set_data(m, E_kin[n])

    # Aktualisiere die Ausgleichsgerade.
    steigung, yabschnitt = np.polyfit(m, E_kin[n], 1)
    plot_gerade.set_data(m, yabschnitt + steigung * m)

    return kreise + [plot_energie, plot_gerade]
